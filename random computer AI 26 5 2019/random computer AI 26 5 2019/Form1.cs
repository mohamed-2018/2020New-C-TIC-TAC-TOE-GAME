...
namespace random_computer_AI_26_5_2019
{
    public partial class Form1 : Form
    {
        ...
        private void PlayerClick(object sender, EventArgs e)
        {
            ...
            button.Text = "X"; // change the button text to player X
            button.Enabled = false; // disable the button
            button.BackColor = Color.Turquoise; // change the player colour to Cyan
        ...
        private void AImove(object sender, EventArgs e)
        {
            ...
            if (buttons.Count > 0)
            {
                ...
                buttons[index].Text = "O"; // show O on the button
                buttons[index].BackColor = Color.Snow; // change the background of the button dark salmon colour
            ...
            private void Check()
            {
             ...
             else if (!(buttons.Count > 0)) // if there is no button in case empty array of buttons 
            {
                MessageBox.Show("Draw", "NO Winner");
                DrawCounts++;
                label3.Text = "Draw Counts- " + DrawCounts; // update the label 2 for computer wins
            }
        }
        ...
        private void resetGame()
        {
            ...
            {
                if (C is Button && C.Tag == "play")  // in this vase we use tag becuse to identify thye buttons of play it self 
                    // not the button of New Game so we declare the buttons of play with tag name play and we calling them 
                {
                    ((Button)C).Enabled = true; // change them all back to enabled or clickable
                    ((Button)C).Text = ""; // set the text to question mark
                    ((Button)C).BackColor = default(Color); // change the background colour to default button colors
                }
            }
